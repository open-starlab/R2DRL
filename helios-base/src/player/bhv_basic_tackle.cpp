...
bool
Bhv_BasicTackle::isExecutable( PlayerAgent * agent )
{
    const ServerParam & SP = ServerParam::i();
    const WorldModel & wm = agent->world();
    ...
    {
        // v11 or older
        return isExecutableOld( agent );

    }
    else if ( agent->config().version() < 14.0 )
    {
        // v12-13
        return isExecutableV12( agent );
    }

    // v14+
    return isExecutableV14( agent, use_foul );
}
/*-------------------------------------------------------------------*/
/*!
...
bool
Bhv_BasicTackle::isExecutableOld( PlayerAgent * agent )
{
    const WorldModel & wm = agent->world();
    const ServerParam & SP = ServerParam::i();
    double tackle_power = SP.maxTacklePower();
...
bool
Bhv_BasicTackle::isExecutableV12(PlayerAgent * agent)
{   
    static GameTime s_last_execute_time( 0, 0 );
    static bool s_result = false;
    static AngleDeg s_best_angle = 0.0;
...
bool
Bhv_BasicTackle::isExecutableV14( PlayerAgent * agent,
                             const bool use_foul )
{
    return true;
}
