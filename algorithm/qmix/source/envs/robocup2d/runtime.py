...
class Runtime:
    ...
    def initialize_session(self, where: str = "init") -> None:
        ...
        self.log.info(
            ...
            f"base={self.base_port} "
            f"server={self.server_port} "
            f"trainer={self.trainer_port} "
            f"coach={self.coach_port} "
            f"debug={self.debug_port} "
            f"lock={self._port_lock_path}"
        )
    ...
    def start_procs(self) -> None:
        ...
        player_procs, _ = proc.launch_players(
            ...
            capture_logs=self.config.text_logging,
        )
    ...
    def _session_ports(self) -> List[int]:
        ports: List[int] = []
        for port in (
            self.server_port,
            self.trainer_port,
    ...
    def _session_shm_names(self) -> List[str]:
        names: List[str] = []
        if self.coach_shm_id:
            names.append(self.coach_shm_id)
        if self.trainer_shm_id:
            names.append(self.trainer_shm_id)
        if self.player_shm_ids:
            names.extend(self.player_shm_ids.values())
        return names

    def _cleanup_session_residue(
        self,
        run_id: Optional[str],
        shm_names: Optional[List[str]] = None,
        *,
        ...
        if kill_processes:
            P.common._safe(
                proc.kill_run_processes_by_run_id,
                str(run_id),
                log=self.log,
            )
            P.common._safe(
                proc.kill_run_ports_by_run_id,
                self._session_ports(),
                run_id=str(run_id),
                log=self.log,
            )
        ...
        if cleanup_shm:
            seen = set()
            for name in shm_names or []:
                if not name or name in seen:
                    continue
                seen.add(name)
                P.common._safe(ipc.cleanup_shm, name, unlink=True, log=self.log)
            P.common._safe(ipc.cleanup_shm_by_run_id, str(run_id), log=self.log)
    ...
    def restart_session(self, where: str = "restart") -> None:
        ...
        old_names = self._session_shm_names()

        self.kill_all()
        self._cleanup_session_residue(
            old_run_id,
            old_names,
            kill_processes=True,
            cleanup_shm=True,
    ...
    def all_procs_live(self) -> bool:
        ...
        for item in self.procs:
            ...
            if p is None:
                continue
            saw_proc = True
            if P.common._safe(p.poll) is not None:
                return False
        return saw_proc

    def dead_proc_descriptions(self) -> List[str]:
        dead: List[str] = []
        for item in self.procs:
            p = P.common._as_popen(item)
            if p is None:
                continue
            rc = P.common._safe(p.poll)
            if rc is None:
                continue
            kind = getattr(item, "kind", p.__class__.__name__)
            team = getattr(item, "team", "")
            unum = getattr(item, "unum", "")
            label = str(kind)
            if team:
                label += f":{team}"
            if unum:
                label += f":u{unum}"
            dead.append(f"{label}:pid={getattr(p, 'pid', '?')}:rc={rc}")
        return dead
    ...
    def kill_all(self) -> None:
        ...
        for p in popens:
            if p.poll() is None:
                ...
                P.common._safe(p.wait, timeout=0.0)
    ...
    def close(self) -> None:
        ...
        run_id = self.run_id
        shm_names = self._session_shm_names()
        ...
        if self._port_lock_fd is not None or self._port_lock_path is not None:
            P.common._safe(
                proc.release_port_lock,
                self._port_lock_fd,
                self._port_lock_path,
                log=self.log,
            )
