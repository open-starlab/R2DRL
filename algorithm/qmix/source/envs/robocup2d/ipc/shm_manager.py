...
def cleanup_shm_by_run_id(
    ...
    *,
    ...
    if not re.fullmatch(r"[0-9a-fA-F]{32}", run_id):
        return []

    shm_prefix = f"{prefix}{run_id}_"
    cleaned: List[str] = []
    try:
        names = os.listdir("/dev/shm")
    except Exception as exc:
        if log:
            log.info(f"[shm] scan /dev/shm failed run_id={run_id}: {exc!r}")
        return cleaned

    for filename in sorted(names):
        if not filename.startswith(shm_prefix):
            continue
        shm_name = "/" + filename
        cleanup_shm(shm_name, unlink=True, log=log)
        cleaned.append(shm_name)

    if cleaned and log:
        log.info(f"[shm] cleanup_by_run_id run_id={run_id} count={len(cleaned)}")
    return cleaned
...
def zero_all_shm_bufs(
    coach_shms: dict,
    trainer_shms: dict,
    player_shms: dict,
) -> None:
    """
    Zero-fill all shm buffers in-place.
    Keep shm objects and names.
    """

    for shm_dict in (coach_shms, trainer_shms, player_shms):
