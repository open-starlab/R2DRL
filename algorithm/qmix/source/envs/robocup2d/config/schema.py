...
class EnvConfig:
    ...
    def __init__(self, args: Dict[str, Any]):
        ...
        # 兼容旧代码：默认 n1/n2 对称；catalog 场景可覆盖成非对称人数。
        self.n1: int = self._optional_int(args, "n1", self.n)
        self.n2: int = self._optional_int(args, "n2", self.n)
        if self.n1 < 1:
        ...
        self.reset_perception_warmup_cycles: int = int(
            args.get("reset_perception_warmup_cycles", 3)
        )
        if self.reset_perception_warmup_cycles < 0:
            raise ValueError(
                "reset_perception_warmup_cycles must be >= 0, "
                f"got {self.reset_perception_warmup_cycles}"
            )
        self.step_trainer_noop: bool = self._optional_bool(
            args, "step_trainer_noop", False
        )
        self.reset_handshake_debug: bool = self._optional_bool(
            args, "reset_handshake_debug", False
        )
        self.reset_handshake_slow_log_seconds: float = self._optional_float(
            args, "reset_handshake_slow_log_seconds", 1.0
        )
        self.use_action_mask: bool = self._optional_bool(
        ...
        self.epv_progress_reward: float = self._optional_float(
            args, "epv_progress_reward", 0.0
        )
        self.epv_progress_scale: float = self._optional_float(
            args, "epv_progress_scale", 1.0
        )
        self.epv_grid_file: str = self._optional_str(
            args, "epv_grid_file", "EPV_grid.csv"
        ) or "EPV_grid.csv"
        ...
        self.start_catalog_enabled: bool = self._optional_bool(
            args, "start_catalog_enabled", False
        )
        self.start_id: Optional[str] = self._optional_str(args, "start_id", None)
        self.start_index = args.get("start_index", None)
        if self.start_index is not None and not isinstance(self.start_index, int):
            raise TypeError(f"'start_index' must be int, got {type(self.start_index)}")
        ...
        self.freeze_non_controlled: bool = self._optional_bool(
            args, "freeze_non_controlled", False
        )
        self.active_right_unums: List[int] = self._optional_int_list(
            args, "active_right_unums", []
        )
        ...
        self.terminate_on_ball_left_half: bool = self._optional_bool(
            args, "terminate_on_ball_left_half", False
        )
        self.reward_on_ball_left_half: float = self._optional_float(
            args, "reward_on_ball_left_half", 0.0
        )
        self.reward_on_timeout: float = self._optional_float(
            args, "reward_on_timeout", 0.0
        )
        self.reward_on_draw: float = self._optional_float(
            args, "reward_on_draw", 0.0
        )
        ...
        if self.epv_progress_reward < 0.0:
            raise ValueError(
                f"epv_progress_reward must be >= 0.0, got {self.epv_progress_reward}"
            )
        if self.epv_progress_scale < 0.0:
            raise ValueError(
                f"epv_progress_scale must be >= 0.0, got {self.epv_progress_scale}"
            )
        if not self.epv_grid_file:
            raise ValueError("epv_grid_file must not be empty")
        if len(set(self.active_right_unums)) != len(self.active_right_unums):
            raise ValueError(
                f"active_right_unums must not contain duplicates, got {self.active_right_unums}"
            )
        for unum in self.active_right_unums:
            if not (1 <= unum <= self.n2):
                raise ValueError(
                    f"active_right_unums values must be between 1 and {self.n2}, got {unum}"
                )
    ...
    def _optional_int(self, args: Dict[str, Any], key: str, default: int) -> int:
    ...
    def _optional_int_list(
        self, args: Dict[str, Any], key: str, default: List[int]
    ) -> List[int]:
        if key not in args or args[key] is None:
            return list(default)
        val = args[key]
        if not isinstance(val, list):
            raise TypeError(f"'{key}' must be list[int], got {type(val)}")
        out: List[int] = []
        for item in val:
            if not isinstance(item, int):
                raise TypeError(
                    f"'{key}' must be list[int], got item {item!r} of type {type(item)}"
                )
            out.append(item)
        return out
