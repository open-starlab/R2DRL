...
def pick_ports(auto_start, auto_end, auto_step,
    ...
    for base in range(auto_start, auto_end, auto_step):
        ...
        release_port_lock(fd, path, unlink_file=True)
...
def release_port_lock(
    ...
    *,
    ...
    if unlink_file and path:
        try:
            os.unlink(path)
            if log:
                log.info(f"[ports] released lock path={path}")
        except FileNotFoundError:
            pass
        except Exception as exc:
            if log:
                log.info(f"[ports] release lock failed path={path} err={exc!r}")
