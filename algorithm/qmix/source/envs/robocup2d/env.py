...
from .start_sampler import ScenarioStartSampler, apply_catalog_launch_profile
    ...
    def __init__(self, cfg="r2drl.yaml", **env_args):
        ...
        if self.env.config.trajectory_path:
            self.start_sampler = ScenarioStartSampler(
                ...
                start_catalog_enabled=self.env.config.start_catalog_enabled,
                start_id=self.env.config.start_id,
                start_index=self.env.config.start_index,
            )
...
class Robocup2dEnv:
    def __init__(self, cfg="robocup.yaml", **env_args):
        ...
        raw_env_args = load_env_args(cfg, env_args)
        apply_catalog_launch_profile(raw_env_args)
        self.config = EnvConfig(raw_env_args)
        ...
        if self.config.lib_paths:
            lib_paths = [str(p) for p in self.config.lib_paths]
            preferred = [p for p in lib_paths if p.endswith("/librcsc/rcsc/.libs")]
            rest = [p for p in lib_paths if p not in preferred]
            system_libs = ["/lib/x86_64-linux-gnu", "/usr/lib/x86_64-linux-gnu"]
            merged = ":".join(preferred + rest + system_libs)
            if base_ld:
        ...
        self._default_reset_reposition_count = 0
    ...
    def _controlled_agent_count(self) -> int:
        return min(int(self.config.init_n), int(self.config.n1))
    ...
    def _apply_reset_start(self):
        ...
        self.agents.set_mask_n(self._controlled_agent_count())
    ...
    def _sync_control_count(self):
        return self.agents.set_mask_n(self._controlled_agent_count())

    def _next_default_reset_kickoff_side(self) -> str:
        # The initial server kickoff is not trainer-repositioned. The first
        # manual default reset therefore corresponds to the opposite kickoff,
        # then alternates for fairness across timeout-driven benchmark episodes.
        return "right" if (self._default_reset_reposition_count % 2 == 0) else "left"
    ...
    def _runtime_all_procs_live(self, *, context: str) -> bool:
        ok = self.runtime.all_procs_live()
        if (not ok) and self.log:
            dead = ",".join(self.runtime.dead_proc_descriptions())
            self.log.warning(
                "[runtime] dead_process_detected "
                f"context={context} run_id={self.runtime.run_id} "
                f"dead={dead or 'unknown'}"
            )
        return ok

    def _reset_startup_ready_timeout(
        self,
        wait_ready_timeout: float,
        trainer_ready_timeout_ms: float,
    ) -> float:
        trainer_seconds = float(trainer_ready_timeout_ms) / 1000.0
        if trainer_seconds <= 0.0:
            trainer_seconds = 10.0
        return max(1.0, min(float(wait_ready_timeout), trainer_seconds))
    ...
    def _is_ball_left_half(self, ball_x: float) -> bool:
        return float(ball_x) < 0.0
    ...
    def reset(self):
        ...
        did_restart = self._need_restart or (
            not self._runtime_all_procs_live(context="reset_entry")
        )
        ...
        startup_ready_timeout = self._reset_startup_ready_timeout(
            wait_ready_timeout,
        ...
        for attempt in range(max_attempts):
            ...
            if not self._runtime_all_procs_live(
                context=f"reset_attempt_start turn={self.turn_count} attempt={attempt + 1}/{max_attempts}"
            ):
                if attempt + 1 < max_attempts:
                    did_restart = True
                    restart_reason = f"dead_procs_attempt_{attempt + 1}"
                    continue
                raise P.common.ShmProtocolError("RoboCup process died before reset")
            ...
            default_reset_kickoff_side = None
            ...
            if use_default_goal_restart:
                ...
                if not self._wait_all_ready_with_timeout(
                    ...
                    context=f"reset_startup_ready_precheck turn={self.turn_count} attempt={attempt + 1}/{max_attempts}",
                ):
                    if attempt + 1 < max_attempts:
                        did_restart = True
                        restart_reason = f"reset_startup_ready_timeout_attempt_{attempt + 1}"
                        continue
                    raise P.common.ShmProtocolError("Not READY Before Reset Startup")
                ...
                if self.config.use_custom_start:
                    ...
                    default_reset_kickoff_side = self._next_default_reset_kickoff_side()
                    ready = self.agents.reset_default(
                        wait_timeout=wait_ready_timeout,
                        kickoff_side=default_reset_kickoff_side,
                    )
                if not ready:
                    if attempt + 1 < max_attempts:
                        did_restart = True
            ...
            if (not self.config.use_custom_start) and default_reset_kickoff_side is not None:
                self._default_reset_reposition_count += 1
                if self.log:
                    self.log.info(
                        "[reset_default_kickoff] "
                        f"turn={self.turn_count} "
                        f"count={self._default_reset_reposition_count} "
                        f"kickoff_side={default_reset_kickoff_side}"
                    )
            ...
            return {
                ...
                "initial_episode_epv": float(self.agents.initial_episode_epv),
                "max_episode_epv": float(self.agents.max_episode_epv),
                "max_epv_improvement": float(
                    self.agents.max_episode_epv - self.agents.initial_episode_epv
                ),
    ...
    def step(self, actions):
        ...
        # Trainer is only needed for reset/reposition. Submitting a trainer
        # noop on every play step can leave the step barrier waiting on a
        # trainer ack even after all players have completed their actions.
        if self.config.step_trainer_noop:
        ...
        ball_x = float(ball[0])
        ball_y = float(ball[1])
        epv_reward = float(self.agents.update_episode_epv(position=(ball_x, ball_y)))
        epv_completion_reward = 0.0
        ball_out_of_bounds = self._is_ball_out_of_bounds(ball_x, ball_y)
        ball_left_half_now = self._is_ball_left_half(ball_x)
        reward = epv_reward
        self.done = 0
        ball_out = 0
        ball_left_half = 0
        ...
        if goal == 1:
            epv_completion_reward = float(self.agents.complete_episode_epv())
            reward += epv_completion_reward
            reward += 1.0
            self.score[0] += 1
            self.agents.coach.clear_goal_flag()
        ...
        if (
            (not self.done)
            and self.config.terminate_on_ball_left_half
            and ball_left_half_now
        ):
            self.done = 1
            ball_left_half = 1
            reward += float(self.config.reward_on_ball_left_half)
            terminal_reason = "ball_left_half"
        ...
        if timeout or self._need_restart:
            ...
            if self._need_restart:
                terminal_reason = "restart"
            else:
                score_diff_now = int(self.score[0] - self.score[1])
                reward += float(self.config.reward_on_timeout)
                if score_diff_now == 0:
                    reward += float(self.config.reward_on_draw)
                terminal_reason = "timeout"

        score_diff = int(self.score[0] - self.score[1])
        win = int(self.done and score_diff > 0)
        lose = int(self.done and score_diff < 0)
        # A RoboCup match timeout is the real terminal state, not a PyMARL
        # artificial time-limit truncation. Keep timeout for logging, but make
        # episode_limit false so TD targets do not bootstrap past the match.
        episode_limit_truncated = 0

        info = {
            "win": win,
            "lose": lose,
            "timeout": int(timeout),
            "episode_limit": episode_limit_truncated,
            "score_left": int(self.score[0]),
            "score_right": int(self.score[1]),
            "score_diff": score_diff,
            "ball_out": ball_out,
            "ball_left_half": ball_left_half,
            ...
            "epv_step_reward": float(epv_reward),
            "epv_goal_completion_reward": float(epv_completion_reward),
            "epv_progress_scale": float(getattr(self.config, "epv_progress_scale", 1.0)),
    ...
    def get_env_info(self):
        return {
            "n_agents": int(self._controlled_agent_count()),
