...
class Runner(object):
    ...
    def save(self, episode=0):
        """Save current and step-numbered policy checkpoints."""
        if self.algorithm_name == "mat" or self.algorithm_name == "mat_dec":
            ...
            torch.save(actor_state_dict, os.path.join(str(self.save_dir), "actor.pt"))
            torch.save(critic_state_dict, os.path.join(str(self.save_dir), "critic.pt"))

            try:
                checkpoint_step = int(episode)
            except (TypeError, ValueError):
                checkpoint_step = 0

            if checkpoint_step > 0:
                checkpoint_dir = os.path.join(str(self.save_dir), f"step_{checkpoint_step}")
                os.makedirs(checkpoint_dir, exist_ok=True)
                torch.save(actor_state_dict, os.path.join(checkpoint_dir, "actor.pt"))
                torch.save(critic_state_dict, os.path.join(checkpoint_dir, "critic.pt"))
