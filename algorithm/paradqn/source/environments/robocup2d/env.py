...
class Robocup2dEnv:
    ...
    def _cache_transition_payload(self, include_state: bool = True):
        if include_state:
            self.last_state = np.array(self.agents.state(norm=True), copy=True)
        ...
        if include_state or self.last_state is not None:
            payload["state"] = self.last_state
        return payload
    ...
    def reset(self):
        ...
        for attempt in range(max_attempts):
            ...
            self._cache_transition_payload(include_state=False)
    ...
    def step(self, actions):
        ...
        if goal == 1:
            ...
            reward += float(self.config.reward_on_goal_scored)
            self.score[0] += 1
            self.agents.coach.clear_goal_flag()
            ...
            reward += float(self.config.reward_on_goal_conceded)
            self.score[1] += 1
            self.agents.coach.clear_goal_flag()
            if self.config.terminate_on_goal:
                self.done = 1
