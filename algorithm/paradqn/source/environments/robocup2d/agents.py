...
class Agents:
    ...
    def __init__(
        ...
        *,
        ...
        self._action_debug_enabled = os.environ.get("ROBOCUP_ACTION_DEBUG", "0").lower() not in (
            "0", "false", "no", "off"
        )
    ...
    def _run_reset_perception_warmup(
        ...
        for i in range(cycles):
            self.trainer.reset_players_and_ball(ball, left_players, right_players)
            self._kick_all_players_after_reset()
            if not self.wait_all_ready(
                timeout=timeout,
    ...
    def reset_default(
        ...
        *,
        ...
        self._kick_all_players_after_reset()
        if not self.wait_all_ready(timeout=timeout, context=context):
            return False
    ...
    def reset_custom(self, wait_timeout: float | None = None) -> bool:
        ...
        self._kick_all_players_after_reset()
        if not self.wait_all_ready(timeout=timeout, context="reset_custom_final"):
            return False
        return self._run_reset_perception_warmup(
            timeout,
            "reset_custom_final",
