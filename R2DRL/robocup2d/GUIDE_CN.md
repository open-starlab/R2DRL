...
- `use_custom_start`
  ...
  - `benchmark_mode: scenario` 会强制它为 `true`
  - `benchmark_mode: full_match` 会强制它为 `false`
...
- 我方丢球：`-1`
- 开启 `useMaxEpv` 时：
  - 起始位置的 EPV 只作为 baseline，不直接给 reward
  - 比赛中只有我方真实 `kickable` 时才跟踪 EPV
  - 当 `max_episode_epv` 首次刷新到更大值时，给这次增量 reward
  - 回到更小的 EPV 区域不扣分
- scenario 中对手拿到球权：
  - 默认额外 reward 为 `0`
...
此外，当前 3v3 配置默认还会读取双方球员观测中的真实 `kickable flag`。

- 对于 `3v3`
  - `terminate_on_possession_loss: true` 时
  - 只要任一对方球员真实 `kickable`，episode 就会结束
- 对于 `11v11`
  - 仍然沿用基于距离/最近球员的 `ball_owner_team` 判断
...
- 如果 `useMaxEpv=true`
- 则会在比赛过程中做基于 `max_episode_epv` 的增量 shaping：
  - reset 时记录起始位置 EPV 作为 baseline，不给 reward
  - 只有我方真实 `kickable` 时才更新 EPV
  - 当 `max_episode_epv` 首次提升时，reward 加上这次提升量
  - 回退到更小的 EPV 区域不扣分
...
3. 然后切到 `medium` 和 `hard`
   - 看难度提升后的指标变化
4. 最后再比较 `useMaxEpv=false` 与 `true`
   - 看 EPV 增量 shaping 是否带来收益
