...
class Agents:
    ...
    def __init__(
        ...
        *,
        ...
        self._team1_obs_buf = np.empty((self.config.n1, P.player.STATE_NUM), dtype=np.float32)
        self._team1_mask_buf = np.empty((self.config.n1, self.n_actions), dtype=np.int32)
        if self.config.team1 == "hybrid":
            default_actions = [
                int(player.default_hybrid_action[0])
                for player in self.player_list[:self.config.n1]
            ]
        else:
            default_actions = [
                int(player.default_base_action)
                for player in self.player_list[:self.config.n1]
            ]
    ...
    def update_episode_epv(self) -> float:
        if not self.config.useMaxEpv or not self.true_kickable:
            return 0.0

        prev_max_epv = self.max_episode_epv
        self.current_epv = self.get_ball_position_epv()
        self.max_episode_epv = max(prev_max_epv, self.current_epv)
        return float(self.max_episode_epv - prev_max_epv)
    ...
    def write_base_actions(self, actions: np.ndarray):
        ...
        for idx, p in enumerate(self.player_list):
            if idx < n1:
                ...
                act = int(p.default_base_action)
            p.submit_base_action(act, target_obs_seq=int(p.obs_seq()))
    ...
    def write_hybrid_actions(self, actions: np.ndarray):
        ...
        for idx, p in enumerate(self.player_list):
            ...
            p.submit_hybrid_action(a, u0, u1, target_obs_seq=int(p.obs_seq()))
    ...
    def wait_all_ready(
        ...
        while True:
            ...
            signature = (bool(trainer_idle), tuple(player_ready))

            if trainer_idle and all(player_ready):
                return True
            ...
            ready_count = sum(player_ready)
            stable_for = now - last_change
            can_rescue_players = (
                trainer_idle
                and 0 < ready_count < len(player_ready)
            ...
            if can_rescue_players:
                ...
                for is_ready, player in zip(player_ready, self.player_list):
                    if not is_ready:
                        continue
                ...
                if pushed > 0:
                    if self.log:
                        self.log.info(
                            ...
                            f"ready_players={ready_count}/{len(player_ready)}"
                        )
                    last_rescue = now
                    last_change = now
            ...
            if now >= t_end:
                if self.log:
                    self.log.info(
                        ...
                        f"trainer={{phase:{self.trainer.phase()}, req_seq:{self.trainer.req_seq()}, done_seq:{self.trainer.done_seq()}}} "
    ...
    def _kick_all_players_after_reset(self) -> None:
        """
        After a trainer-driven reposition, force all players to consume one
        fallback action so the synchronized server can advance the cycle.
        This mirrors the old reset flow and prevents trainer reset requests
        from staying pending while everyone is otherwise READY.
        """
        is_hybrid = (self.config.team1 == "hybrid")
        for p in self.player_list:
            p.take_empty_action(is_hybrid=is_hybrid)
            
    def reset_default(self, wait_timeout: float | None = None) -> bool:
        ...
        return self.wait_all_ready(timeout=timeout, context="reset_default")
    ...
    def reset_custom(self, wait_timeout: float | None = None) -> bool:
        ...
        for i in range(5):
            ...
            self._kick_all_players_after_reset()
            if not self.wait_all_ready(timeout=timeout, context="reset_custom_zero"):
                return False
        ...
        self.write_all_body_targets(custom_body_angles)
        self._kick_all_players_after_reset()
        return self.wait_all_ready(timeout=timeout, context="reset_custom_final")
    ...
    def get_team1_obs(self, norm: bool = True, zero_inactive: bool = True):
        ...
        np.copyto(out, full_obs[:self.config.n1])

        if zero_inactive and (not self.agent_mask.all()):
            out[~self.agent_mask] = 0.0
    ...
    def get_team1_avail_actions(self):
        ...
        np.copyto(out, full_mask[:self.config.n1])
        inactive_idx = np.flatnonzero(~self.agent_mask)

        if inactive_idx.size > 0:
            out[inactive_idx] = 0
            out[inactive_idx, self._team1_default_actions[inactive_idx]] = 1
