...
- `full-match`: keep playing after goals, preserve the server's natural kickoff/goal restart flow, and report final `score_diff` at timeout.
- `scenario`: use custom/scenario starts and, in the current 3vs3 presets, terminate on goal, possession loss, ball out, or timeout, then report `success`.
...
- Reward:
  ...
  - scoring reward + EPV shaping: `*_full_match_{easy|medium|hard}_epv`
