...
底层 reset 逻辑在 `Robocup2dEnv.reset()`。当前实现不是“只有少数情况才 reset”，而是每次 `env.reset()` 都会忠实执行一次 reset，只是 `use_custom_start=false` 时有一个“保留自然开球/进球后 restart”的特例。

可以把逻辑概括成：

```python
benchmark_mode == "scenario"   -> use_custom_start = True
benchmark_mode == "full_match" -> use_custom_start = False

if (not use_custom_start) and prev_terminal_reason in ("init", "goal_scored", "goal_conceded"):
    # 不调用 trainer 重新摆位
    # 保留 server 自己的自然 kickoff / goal restart
    wait_all_ready()
else:
    if use_custom_start:
        reset_custom()
...
|---|---|
| true | 每次 `reset()` 都使用 `reset_custom()`，也就是 scenario/custom 起点 |
| false | 默认使用普通比赛 reset；但如果上一回合原因是 `init` / `goal_scored` / `goal_conceded`，则保留 server 的自然 restart，不调用 `reset_default()` |

注意：

- `use_custom_start` 只控制底层 reset 是否使用已经配置好的 custom 起点。它不会自己从轨迹文件采样。采样由 `R2DRL` 或 `parallel_runner.py` 完成。
- `test_mode` 不改变 reset 类型，它只表示当前是训练还是测试/评估。
- `scenario` / `full_match` 两种 `benchmark_mode` 会在代码里自动归一化 `use_custom_start`，避免 YAML 和运行语义不一致。
...
它不只是日志标签。当前代码会用它约束 reset 语义：

- `scenario`
  - 强制 `use_custom_start = true`
- `full_match`
  - 强制 `use_custom_start = false`
- `train`
- `actionspace`
  - 不做强制覆盖，按 YAML 里的 `use_custom_start` 执行
...
- 当 `benchmark_mode: scenario` 时，代码会自动把它归一化为 `true`
- 当 `benchmark_mode: full_match` 时，代码会自动把它归一化为 `false`
- 像 `3vs3 actionspace` 这类不是 `scenario/full_match` 的 preset，则仍然按 YAML 中的 `use_custom_start` 执行
...
- reset 时把起始位置 EPV 记为 baseline，但不直接给 reward
- 只有我方真实 `kickable` 时才更新 EPV
- 当 `max_episode_epv` 首次刷新到更大值时，reward 加上这次增量
- 回退到更小的 EPV 区域不扣分
- timeout 时不再额外补整段 `max_episode_epv`
- `info` 中输出：
  - `max_episode_epv`
  - `useMaxEpv`

这个开关对 `11vs11` 和 `3vs3` 都生效。
...
当前 `3vs3 scenario` 配置默认设为 `true`。

`11vs11 full match` 通常设为 `false`。
...
- `3vs3`
  - 使用对方球员观测中的真实 `kickable flag`
  - 只要任一对方球员真实 `kickable`，就会触发 `possession_loss`
- `11vs11`
  - 仍然使用 `ball_owner_team` 的距离/最近球员推断

`11vs11` 配置通常设为 `false`。

### 5.10 terminate_on_ball_out

```yaml
terminate_on_ball_out: true
```

球出边线，或越过端线且不在球门口范围内时，是否终止 episode。

当前实现的几何判定是：

- `abs(ball_y) > half_width`
- 或 `abs(ball_x) > half_length` 且 `abs(ball_y) > goal_y`

也就是说，这里认的是“几何上的 ball out”，不是所有你视觉上觉得已经死球的比赛事件。

当前 `3vs3 scenario` 配置通常设为 `true`。

### 5.11 trajectory_path
...
并且 `ParallelRunner.reset(test_mode=True)` 也会像训练一样先从 `start_sampler` 采样起点，再发给各个 worker。
...
没有直接关系，但两者会叠加影响 reward 轨迹。

- scenario 起点由 `ScenarioStartSampler` 控制
- MaxEPV 由 `useMaxEpv` 控制
- 起始位置 EPV 只会作为 baseline，不会直接发奖励
- 真正的 EPV reward 来自比赛过程中 `max_episode_epv` 的新增量
